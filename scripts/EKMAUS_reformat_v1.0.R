...
#  ifelse(set[k]<tl,
  ...
  tsp<-t1[set[k]]
  tabsep<-"\t"
  tabsep<-stri_join(rep(tabsep,tabm),collapse = "")
  t2$edit1[tstart]<-paste0("*",tsp,":",tabsep,paste(t1[tstart1:tlast],collapse  = " "))
...
t2<-data.frame(id=1:length(t1),edit1=0,edit2=0)
t3<-getlines(t1,t2,m6,4)
t4<-t3
if(!int2true)
   t4<-getlines(t1,t3,m7,4)
