...
d6safe<-d6
getwd()
write.csv(d6,"local/HU-LX/SES/sesDB010_d6safe.csv")
###### finalise
colnames(d6)
...
# reread DB
#d7
m1<-grep("(SES)",d7$token)
d7$token[m1[3]]
m4<-grep("(</SES)",d7$token) #transcript end
# d7$token[m2[1]]
m3<-grep("(<SES id)",d7$token) #transcript start
m5<-grep("(SES_.*)(SkE)",d7$token,value = T)
#m5<-grepl("(SES_.*)(SkE)",d7$token)
m6<-gsub(".*(SES_.*)(_SkE).*","\\1",m5) #kids
d8<-d7
...
##############################
  ...
  # ms7<-grep("(#[A-Z]{3,3}|0[A-Z]{1,2})",sent1,value = T)
ms6<-head(unique(ms5))
ms6
# x<-ms6
...
###chk for ruths problem of doubles tokens

d10<-read.csv("local/HU-LX/SES/sesDB010c.csv")
d10$turn[15478:19212]
a<-d10$turn[d10$token=="mit"]
b<-unique(a)
a[1:100]
###
d1c<-read_delim("local/HU-LX/SES/ses_vert.csv")
#"mit" outputs 409 entries both in ske vert.csv and my db

d11<-d10
dt<-d11$turn[d11$interview=="GDD"]
du<-unique(dt)
dt[1:10]
m1<-grep("Fuehrerhaus",d11$token)
d11$turn[m1[2:3]]
}