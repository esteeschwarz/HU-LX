git_folder <- Sys.getenv("GIT_TOP")
hkw_folder <- Sys.getenv("HKW_TOP")
hkw_folder<-"~/boxHKW/UNIhkw"
#r_files_git <- list.files(git_folder, pattern = "\\.R$|\\.r$", recursive = TRUE, full.names = TRUE)
#r_files_hkw <- list.files(hkw_folder, pattern = "\\.R$|\\.r$", recursive = TRUE, full.names = TRUE)
pattern = "\\.R$|\\.r$|\\.Rmd$|\\.rmd$|\\.qmd"
pattern = ".*"
r_files_git <- list.files(git_folder, pattern = pattern, recursive = TRUE, full.names = TRUE)
r_files_hkw <- list.files(hkw_folder, pattern = pattern, recursive = TRUE, full.names = TRUE)
r_files<-c(r_files_git,r_files_hkw)
r_files<-c(r_files_hkw)
#head(r_files)
library(dplyr)
#r_files.s<-r_files[1:10]
# get.scripts<-function(top_folder,n){
#   r_files.s<-r_files[1:n]
#   df <- lapply(r_files.s, function(f) {
#   lines <- readLines(f, warn = FALSE)
#   data.frame(
#     location = f,
#     line = seq_along(lines),
#     code = lines,
#     stringsAsFactors = FALSE
#   )
# }) %>% bind_rows()
# }
# lines<-1:10
# seq_along(lines)
#i<-1
get.scripts.i<-function(rfiles,n){
  r_files.s<-r_files[1:n]
  library(tools)
  
df <- bind_rows(
  lapply(seq_along(r_files.s), function(i) {
   # lines <- readLines(r_files.s[i], warn = FALSE)
    finfo<-file.info(r_files.s[i])
    data.frame(
      id = i,
      type = file_ext(r_files[i]),
      location = r_files[i],
      size = finfo$size,
      created = finfo$ctime,
      modified = finfo$mtime,
    #  line = seq_along(lines),
     # code = lines,
      stringsAsFactors = FALSE
    )
  })
)
}
#system.time(get.scripts(top_folder,10))
#system.time(get.scripts.i(r_files,10))

save.db<-function(r_files){
df<-get.scripts.i(r_files,length(r_files))
print(r_files[1:20])
r.scripts.db<-df
save(r.scripts.db,file = paste0(Sys.getenv("HKW_TOP"),"/sys/filesHKW_DB.RData"))
#head(df)
return(df)
}
#########
# query
q<-"dependencies"
#load(paste0(Sys.getenv("HKW_TOP"),"/R/R-scripts-DB.RData"))

get.com<-function(df){
  m<-grep("^#",df$code)
  m2<-grep("^ *?#",df$code)
  #m3<-grep("^#",df$code)
  m4<-c(m,m2)
  m5<-c(m2,m)
}
# head(df$code[m2],1000)
# head(df$code[get.com(df)],1000)

# df$comment<-F
# df$comment[get.com(df)]<-T
# com<-T
# q
q<-"AS204"
df<-inv.db
type<-".*"
column<-"location"
get.q<-function(q,df,column,type){
#  df<-data.frame(df)
  #ifelse(com,code<-df$code,code<-df$code[df$comment])
  m<-grep(q,df[,column])
  mna<-is.na(df$type)
  sum(mna)
  df$type[mna]<-""
  m2<-grepl(type,df$type[m])
  sum(m2)
  m3<-m%in%m[which(m2)]
  sum(m3)
  #print(df[m[m3]])
  return(m[m3])

  }

#mc<-get.q("readLines",r.scripts.db,T)
#mc<-get.q("readtext",r.scripts.db,T)
#mc<-get.q("keywords",df,T)
print(r_files[1:20])
#inv.db<-save.db(r_files)
load(paste0(Sys.getenv("HKW_TOP"),"/sys/filesHKW_DB.RData"))
colnames(inv.db)
inv.db[1:10,]
##########################################
mc<-get.q("AS204",inv.db,"location","pdf")
inv.db$location[mc]
##########################################
