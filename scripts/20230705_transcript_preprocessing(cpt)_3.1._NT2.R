...
##################################
#13275.messy NT2 files first clean
getwd()
linecor2<-function(k,filelist){
  cc<-readtext(paste(dirtext,filelist[k],sep = "/"))
  cc1<-cc$text
  regx<-"[^A-Za-z0-9;,\\.\\(\\),=&+\\-\\: ]"
  repl<-"#00#"
  cc2<-gsub(regx,repl,cc1)
dirouttxt<-paste(dirtext,"SES_basic_txt",sep = "/")
dir.create(dirouttxt)
kids<-strsplit(filelist,"\\.")
#kids[[2]][1]
corfilename<-paste0(kids[[k]][1],"_cleantxt.txt")
#ns<-paste0(filelist[k],"_cleantxt")
writeLines(cc2,)
}
cc2<-linecor2(2,filelist1)
###############################
